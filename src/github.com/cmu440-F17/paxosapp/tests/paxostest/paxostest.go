...
import (
	...
	"github.com/cmu440-F17/paxosapp/tests/proxy"
)
...
const loggingFile = "test_results_paxostest.txt"
...
func runTest(t testFunc, doneChan chan bool) {
	...
	if pass {
		fmt.Println("PASS")
		LOGTEST.Println("\"" + t.name + "\":" + strconv.Itoa(t.points))
		passCount++
	} else {
		LOGTEST.Println("\"" + t.name + "\":0")
		failCount++
	}
}
...
func main() {
	btests := []testFunc{
		{"testSingleProposerSingleProposal", 4, testSingleProposerSingleProposal},
		{"testSingleProposerUpdateKey", 4, testSingleProposerUpdateKey},
		{"testGetNextProposalNumber", 4, testGetNextProposalNumber},
		{"testMultipleSequentialProposers", 4, testMultipleSequentialProposers},
	}
	...
	// Create logging file
	logf, err := os.OpenFile(loggingFile, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
		LOGE.Fatalf("Error opening log file: ", err)
	}
	defer logf.Close()
	LOGTEST = log.New(logf, "", 0)
